...
export class IntegratedFortuneService {
  ...
  static async calculateIntegratedFortune(
    ...
    // 生成投資建議
    const investmentAdvice = this.generateInvestmentAdvice(personalBaZi, lunarData, elements, scores)
   ...
   * 計算五行能量
   */
  private static calculateElementEnergy(personalBaZi: any, lunarData: LunarData, profile?: UserProfileCompat) {
   ...
   * 計算運勢分數
   */
  private static calculateFortuneScores(personalBaZi: any, lunarData: LunarData, elements: any) {
    let overallScore = 50
    ...
    // 農民曆宜忌影響
    if (lunarData.yi.includes('交易') || lunarData.yi.includes('開市') || lunarData.yi.includes('求財')) {
      investmentScore += 20
      wealthScore += 15
    }
    if (lunarData.ji.includes('交易') || lunarData.ji.includes('開市') || lunarData.ji.includes('求財')) {
      investmentScore -= 20
      wealthScore -= 15
    }
   ...
   * 生成投資建議
   */
  private static generateInvestmentAdvice(personalBaZi: any, lunarData: LunarData, elements: any, scores: any) {
    ...
    // 根據五行關係調整建議
    const dominantElement = Object.entries(elements).reduce((max, [key, value]) => 
   ...
   * 計算台股交易分析
   */
  private static calculateStockTradingAnalysis(personalBaZi: any, lunarData: LunarData, date: Date) {
    // 獲取台股交易狀態（檢查當前時間和日期）
    ...
    // 計算下個交易日（用於交易日提醒）
    const nextTradingDay = stockTradingStatus.nextTradingDay || 
   ...
   * 計算時間分析
   */
  private static calculateTimeAnalysis(personalBaZi: any, lunarData: LunarData) {
   ...
   * 計算方位分析
   */
  private static calculateDirectionAnalysis(personalBaZi: any, lunarData: LunarData) {
    // 根據天干地支計算吉方
    const dayGan = personalBaZi.dayGanZhi[0]
    const dayZhi = personalBaZi.dayGanZhi[1]
   ...
   * 生成活動建議
   */
  private static generateActivitiesAdvice(personalBaZi: any, lunarData: LunarData) {
   ...
   * 生成特殊建議
   */
  private static generateSpecialAdvice(personalBaZi: any, lunarData: LunarData, elements: any) {
  ...
  private static calculateBestTradingHours(personalBaZi: any, lunarData: LunarData): string[] {
    // 計算交易時段內的最佳時間
    const tradingHours = []
    const luckyZhi = this.getLuckyZhiForZodiac(personalBaZi.zodiac)
  ...
  private static calculateLuckyColors(personalBaZi: any, lunarData: LunarData): string[] {
  ...
  private static calculateLuckyNumbers(personalBaZi: any, lunarData: LunarData): number[] {
    const element = personalBaZi.element
