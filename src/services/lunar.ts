...
class LunarService {
  ...
  private getFestivals(lunar: LunarObject, solar: SolarObject): string[] {
  ...
  private getJieQi(solar: SolarObject): string {
    try {
      // 嘗試不同的節氣方法名稱
  ...
  private getSafeString(obj: LunarObject | SolarObject, methodName: string): string {
    try {
  ...
  private getSafeArray(obj: LunarObject | SolarObject, methodName: string): string[] {
    try {
      if (obj && typeof obj[methodName] === 'function') {
        const result = obj[methodName]()
