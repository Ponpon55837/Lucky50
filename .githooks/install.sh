...
echo "  • pre-push - 推送前執行 TypeScript 類型檢查"
...
echo "    - 驗證分支命名規範（僅警告）"
echo "    - 執行 unit test（vitest run --changed）"
echo "  • commit-msg"
echo "    - 驗證提交訊息格式是否符合 Conventional Commits 規範"
echo "    - 目前為警告模式，不會阻止提交"
echo "  • pre-push"
echo "    - 推送前執行 TypeScript 類型檢查（vue-tsc --noEmit）"
echo ""
echo "🧪 測試建議："
...
echo "     → 應該會看到錯誤訊息阻止提交"
echo ""
echo "  2. 測試 pre-commit unit test："
echo "     git checkout -b test/your-name/test-hooks"
echo "     git commit --allow-empty -m \"feat: 測試提交\""
echo "     → 應該會執行 vitest run --changed"
echo ""
echo "  3. 測試 pre-push type check："
echo "     git push"
echo "     → 應該會執行 vue-tsc --noEmit"
echo ""
echo "💡 提示："
echo "  - 如果需要重新安裝，再次執行此腳本即可"
echo "  - 如果需要臨時跳過 hook，使用：git commit --no-verify 或 git push --no-verify"
