...
- `agent`：提供 **Lucky50 專案特定**的業務與架構背景
...
```
請載入 code-standards skill
```

```
請同時載入 code-standards 和 git-workflow skills
```

### 檢查是否符合規範

```
請載入 code-standards skill 並檢查這個組件是否符合規範
```
...
| 版本  | 日期       | 說明                                             |
| ----- | ---------- | ------------------------------------------------ |
| 2.3.0 | 2026-07-18 | 文件一致性修正（目錄樹、ghost names、symlink）   |
